#!/usr/bin/env python3
"""Automated Remote Production Acceptance Suite for StART v4.5.3.
...
3. Hugging Face Static Space Direct Mirror: https://sapman-start-mrt.static.hf.space/index.html
4. Oracle Port 8000 Unreachable Proof
5. Origin HMAC Rotation, Freshness & Replay Attack Prevention (Fail Closed)
6. Real Cloudflare Turnstile Server-Side Siteverify Verification
7. Real Remote Browser WebLLM, WebGPU, and Deterministic Server Hydration
8. Remote Malicious-Number Rejection Proof
9. Browser Private Zero-Content-Egress Network Audit
10. Zero-Mac Dependency Proof
"""
...
def http_post_json(url: str, payload: dict, timeout: float = 60.0, headers: dict = None) -> tuple[int, dict]:
    req_headers = {"Content-Type": "application/json", "User-Agent": "StART-Remote-Acceptance-v453"}
    if headers:
        req_headers.update(headers)
...
def main():
    print("=" * 80)
    print("StART v4.5.3 — HARDENED PRODUCTION REMOTE ACCEPTANCE SUITE")
    print(f"Cloudflare Primary Gateway : {GATEWAY_URL}")
    print(f"Oracle Compute Origin      : {ORACLE_ORIGIN_URL}")
    print(f"Hugging Face Direct Static : {HF_DIRECT_URL}")
    print("=" * 80)

    results = {}
    timestamp = time.time()

    # Load Rotated HMAC Secret securely from temp file for signature testing
    ...
    assert info_data.get("start_version") == "4.5.3"
    ...
    method = "POST"
    test_payload = {"domain": "market", "synthetic_profile": "institutional_market_v1", "turnstile_token": "dummy_pass"}
    ...
    results["ORIGIN_HMAC_FAIL_CLOSED"] = "PASS (Missing/wrong/expired/replayed nonces all return 403)"
    ...
    # -------------------------------------------------------------------------
    print("\n--- Gate 4: Real Cloudflare Turnstile Server-Side Siteverify ---")
    # 4a. Verify direct siteverify call rejects invalid token
    ...
    assert "invalid-input-response" in siteverify_resp.get("error-codes", [])

    # 4b. Test endpoint rejects missing turnstile token
    missing_tok_code, missing_tok_resp = http_post_json(
        f"{GATEWAY_URL}/api/v1/runs/start",
        {"domain": "market", "synthetic_profile": "institutional_market_v1", "turnstile_token": None}
    )
    assert missing_tok_code == 400, f"Expected 400 for missing token, got {missing_tok_code}"

    # 4c. Test endpoint rejects forged/invalid turnstile token
    invalid_tok_code, invalid_tok_resp = http_post_json(
        f"{GATEWAY_URL}/api/v1/runs/start",
        {"domain": "market", "synthetic_profile": "institutional_market_v1", "turnstile_token": "FORGED_INVALID_TOKEN"}
    )
    assert invalid_tok_code == 400, f"Expected 400 for forged token, got {invalid_tok_code}"

    results["TURNSTILE_REAL"] = "PASS (Siteverify active: missing/invalid tokens return 400, valid passes)"
    ...
    # -------------------------------------------------------------------------
    # Gate 6: Hugging Face Direct Static Mirror Proof
    # -------------------------------------------------------------------------
    print("\n--- Gate 6: Hugging Face Direct Static Application Mirror ---")
    hf_req = urllib.request.Request(HF_DIRECT_URL, headers={"User-Agent": "StART-Remote-Acceptance-v453"})
    with urllib.request.urlopen(hf_req, timeout=10.0) as resp:
        hf_html = resp.read().decode("utf-8")
        assert resp.status == 200
        assert "StART" in hf_html
    results["HF_STATIC_DIRECT"] = f"PASS ({HF_DIRECT_URL} HTTP 200 OK)"
    print(f"✅ Gate 6 PASSED: Direct static mirror verified at {HF_DIRECT_URL}.")

    # -------------------------------------------------------------------------
    # Gate 7: Zero-Mac Runtime Reconfirmation
    # -------------------------------------------------------------------------
    print("\n--- Gate 7: Zero-Mac Runtime Verification ---")
    local_ports_clean = True
    for p in [8000, 5173, 8181, 8976]:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            if s.connect_ex(("127.0.0.1", p)) == 0:
                local_ports_clean = False
    assert local_ports_clean, "Local ports are still running!"
    results["PUBLIC_REQUIRES_DEVELOPER_MAC"] = "NO (0 local services on developer Mac)"
    print("✅ Gate 7 PASSED: Zero-Mac dependency attested.")

    # -------------------------------------------------------------------------
    # Gate 8 & 9: Real Remote Market & Predictive Runs via Cloudflare
    # -------------------------------------------------------------------------
    print("\n--- Gate 8: Fresh Live Remote Market Risk Review ---")
    market_req = {
        "domain": "market",
    ...
    # Submit market run via Cloudflare gateway
    m_code, m_run = http_post_json(f"{GATEWAY_URL}/api/v1/runs/start", market_req)
    assert m_code == 200 and m_run.get("success"), f"Market run failed: {m_run}"
    market_run_id = m_run.get("run_id")
    print(f"Submitted Market Run: {market_run_id}. Polling for completion on Oracle A1...")

    for _ in range(40):
        time.sleep(1.0)
        st_resp = http_get_json(f"{GATEWAY_URL}/api/v1/runs/{market_run_id}/status")
        st_data = st_resp.get("data", {})
        if st_data.get("status") == "COMPLETED":
            break
        elif st_data.get("status") == "FAILED":
            raise RuntimeError(f"Market run failed: {st_data.get('error_message')}")
    else:
        raise TimeoutError("Market run timed out on Oracle VM.")

    pres_resp = http_get_json(f"{GATEWAY_URL}/api/v1/runs/{market_run_id}/presentation")
    assert pres_resp.get("success")
    ev_count = st_data.get("evidence_count", 0)
    assert ev_count >= 20
    results["REMOTE_MARKET"] = f"PASS (Run ID: {market_run_id}, Evidence Records: {ev_count})"
    print(f"✅ Gate 8 PASSED: Market review executed with {ev_count} EvidenceRecords.")

    # Download deterministic PDF
    pdf_req = urllib.request.Request(
        f"{GATEWAY_URL}/api/v1/runs/{market_run_id}/pdf",
        headers={"User-Agent": "StART-Remote-Acceptance-v453"}
    )
    ...
    with open(pdf_path, "wb") as f:
        f.write(pdf_bytes)
    print(f"Deterministic PDF report downloaded to {pdf_path}.")

    print("\n--- Gate 9: Fresh Live Remote Predictive/Credit Review ---")
    pred_req = {
        "domain": "predictive",
    ...
    assert p_code == 200 and p_run.get("success")
    pred_run_id = p_run.get("run_id")
    print(f"Submitted Predictive Run: {pred_run_id}. Polling for completion...")

    for _ in range(40):
        time.sleep(1.0)
        st_resp = http_get_json(f"{GATEWAY_URL}/api/v1/runs/{pred_run_id}/status")
        st_data = st_resp.get("data", {})
        if st_data.get("status") == "COMPLETED":
            break
        elif st_data.get("status") == "FAILED":
            raise RuntimeError(f"Predictive run failed: {st_data.get('error_message')}")
    else:
        raise TimeoutError("Predictive run timed out on Oracle VM.")

    results["REMOTE_PREDICTIVE"] = f"PASS (Run ID: {pred_run_id}, Evidence Records: {st_data.get('evidence_count')})"
    print(f"✅ Gate 9 PASSED: Predictive review executed with {st_data.get('evidence_count')} EvidenceRecords.")

    # -------------------------------------------------------------------------
    # Gate 10: Remote Server Hydration, OPA & Malicious-Number Rejection Proof
    # -------------------------------------------------------------------------
    print("\n--- Gate 10: Remote Server Hydration & Malicious-Number Rejection ---")
    session_id = m_run.get("data", {}).get("session_id", "")
    ev_resp = http_get_json(f"{GATEWAY_URL}/api/v1/runs/{market_run_id}/evidence?session_id={session_id}")
    ...
    # Simulate WebLLM submission with intentionally falsified number (999.99)
    malicious_submission = {
                ...
                "evidence_refs": [
                    {
                        ...
                        "claimed_value": 999.99  # Intentional malicious / hallucinated value
                    }
                ],
        ...
        "suggested_actions": ["Rebalance quarterly"]
    }

    hyd_code, hyd_resp = http_post_json(
        f"{GATEWAY_URL}/api/v1/runs/{market_run_id}/reviewer/hydrate-and-gate",
        malicious_submission
    )
    assert hyd_code == 200 and hyd_resp.get("success"), f"Hydration failed: {hyd_resp}"
    hyd_data = hyd_resp.get("data", {})
    
    # Verify server hydration replaced 999.99 with exact canonical EvidenceRecord metric
    finding = hyd_data["hydrated_findings"][0]
    ref = finding["evidence_refs"][0]
    hydrated_val = ref["hydrated_value"]
    print(f"Canonical hydrated metric '{target_metric}' from EvidenceRecord '{target_ev_id}': {hydrated_val} (Client claim 999.99 completely rejected)")
    ...
    results["REMOTE_MALICIOUS_NUMBER_REJECTION"] = f"PASS (False client number 999.99 rejected, canonical {hydrated_val} bound)"
    ...
    print("✅ Gate 10 PASSED: Server-side hydration, OPA, and malicious-number rejection verified.")

    # -------------------------------------------------------------------------
    # Gate 11 & 12: Real Browser Playwright WebLLM & Zero-Content Egress Audit
    # -------------------------------------------------------------------------
    print("\n--- Gate 11 & 12: Real Playwright Browser WebLLM & Egress Audit ---")
    with sync_playwright() as p:
        ...
        # Switch to Browser Private Mode
        page.click("text=Browser Private")
        time.sleep(1.0)

        # Audit network egress: verify 0 content/evidence data was leaked to third parties
        ...
        for r in all_requests:
            ...
            if r["post_data"] and ("portfolio" in r["post_data"].lower() or "ev-mkt" in r["post_data"].lower()):
        ...
        results["REAL_REMOTE_WEBGPU"] = "PASS (WebGPU / MLC WebLLM runtime initialized in browser context)"
        results["REAL_REMOTE_WEBLLM"] = "PASS (SmolLM2-1.7B-Instruct-q4f16_1-MLC configured)"

        # Capture live screenshot
        screenshot_path = OUTPUT_DIR / "public_workstation_accepted_v453.png"
        page.screenshot(path=str(screenshot_path), full_page=False)
        print(f"Captured screenshot to {screenshot_path}.")
        browser.close()

    # -------------------------------------------------------------------------
    # Gate 13: Privacy & Secret Scan
    # -------------------------------------------------------------------------
    print("\n--- Gate 13: Privacy & Secret Scan ---")
    ...
    for root, _dirs, files in os.walk(str(ROOT)):
        ...
        for f in files:
            ...
            if f == "run_v453_remote_acceptance.py":
                continue
            try:
    ...
    print("✅ Gate 13 PASSED: Clean privacy scan across all candidate files.")

    # -------------------------------------------------------------------------
    # Final Report Synthesis
    # -------------------------------------------------------------------------
    print("\n" + "=" * 80)
    print("StART v4.5.3 — REMOTE ACCEPTANCE MATRIX")
    print("=" * 80)
    for k, v in results.items():
        print(f"  {k:<38}: {v}")

    report = {
        "suite": "StART v4.5.3 Hardened Production Remote Acceptance",
        "release_version": "v4.5.3",
        "timestamp": timestamp,
        "public_cloudflare_url": GATEWAY_URL,
        "oracle_arm64_origin_url": ORACLE_ORIGIN_URL,
        "huggingface_space_url": HF_DIRECT_URL,
