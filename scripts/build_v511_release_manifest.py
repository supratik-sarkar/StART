#!/usr/bin/env python3
"""Build Cryptographic Release Freeze Candidate Manifest for StART v5.1.1 (Measured Acceptance)."""
...
def build_candidate_manifest() -> dict[str, Any]:
    ...
    for d in scan_dirs:
        ...
        for r, _, files in os.walk(dp):
            if any(p in ("__pycache__", ".venv-start", "node_modules") for p in Path(r).parts):
                continue
    ...
    manifest_data = {
        "release": "StART v5.1.1 Candidate Freeze",
        "version": "5.1.1",
        "timestamp": time.time(),
    ...
    manifest_file = OUTPUT_DIR / "v511_candidate_manifest.json"
    with open(manifest_file, "w", encoding="utf-8") as f:
        json.dump(manifest_data, f, indent=2)

    print(f"Generated v5.1.1 candidate manifest with {len(manifest_entries)} files: {manifest_file}")
