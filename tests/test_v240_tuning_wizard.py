...
def test_tuning_run_kfold_regression_recurrent():
    ...
    run = run_tuning(
        df, "attrition", features,
        strategy="bounded_random_search", n_trials=3,
        primary_metric="auc_roc", seed=42,
        architecture="lstm", task_type="binary_classification",
        validation="k_fold", k_folds=3,
    )
