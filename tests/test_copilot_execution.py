...
pytest.importorskip("torch", reason="co-pilot execution trains a small model")

from start.modeling.copilot_execution import (  # noqa: E402
    render_copilot_execution_markdown,
    run_copilot_execution,
)
...
def execution(tmp_path_factory):
    ...
    out = str(tmp_path_factory.mktemp("copilot"))
    ex = run_copilot_execution(
...
def test_markdown_render(execution):
    ex, _ = execution
    md = render_copilot_execution_markdown(ex)
