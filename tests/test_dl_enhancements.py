...
def test_gnn_dcn_fitting():
    ...
    # GNN fit
    clf_gnn = TabularDLClassifier(task="binary_classification", family="gnn", epochs=3, random_state=0)
    clf_gnn.fit(df[features], df["is_fraud"])
    proba = clf_gnn.predict_proba(df[features])
    assert proba.shape == (len(df), 2)

    # DCN fit
    clf_dcn = TabularDLClassifier(task="binary_classification", family="dcn", epochs=3, random_state=0)
...
def test_new_activations():
    ...
    for act in ("swish", "mish", "sigmoid", "softplus"):
        clf = TabularDLClassifier(task="binary_classification", activation=act, epochs=2, random_state=0)
...
def test_regression_pipeline_and_preprocessing():
    ...
    clf = TabularDLClassifier(task="regression", family="mlp", epochs=5, random_state=0, winsorize=True)
