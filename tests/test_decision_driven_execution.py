...
def test_rejecting_correlation_pruning_keeps_all_features():
    ...
    honored = [
        t for t in out.trace_log.traces if "honored user rejection" in t.to_dict().get("action_taken", "")
    ]
...
def test_architecture_override_produces_honored_trace():
    ...
    honored = [t for t in out.trace_log.traces if t.to_dict().get("user_decision") == "override honored"]
    assert len(honored) == 1
