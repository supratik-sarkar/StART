...
def test_torch_mlp_class_weight():
    ...
    df_imbalanced = pd.concat([df[df["attrition"] == 0].sample(200, random_state=42),
...
def test_tabular_dl_class_weight():
    ...
    # Binary
    clf_bin = TabularDLClassifier(task="binary_classification", epochs=3, random_state=42, class_weight="balanced")
    ...
    clf_multi = TabularDLClassifier(task="multiclass_classification", epochs=3, random_state=42, class_weight="balanced")
