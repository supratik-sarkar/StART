...
class AgentMessageTrace(BaseModel):
    ...
    alternatives_considered: list[str] = Field(
        default_factory=list, description="Alternative hypotheses evaluated."
    )
    evidence_citations: list[str] = Field(
        default_factory=list, description="Strict references to EV-xxxx hashes."
    )
    confidence_score: float = Field(..., description="Calculated metric stability bounds.")
    action_directive: dict[str, Any] | None = Field(
        None, description="Programmatic instructions for self-healing."
    )
    ...
    def validate_citations(cls, v: list[str]) -> list[str]:
        for citation in v:
            if (
                not citation.startswith("EV-")
                and not citation.startswith("FE-")
                and not citation.startswith("ARCH-")
                and not citation.startswith("TUNE-")
            ):
                raise ValueError(
