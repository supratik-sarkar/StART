...
def review_propensity_demo(
    ...
    """Propensity-style model review demo: 60/20/20 split, feature checks, model + tuning choice."""
    from start.modeling.data import TARGET_COLUMN
    from start.modeling.propensity import PropensityOptions, prompt_options, run_propensity_demo
...
def review_challenge_findings_cmd(
    config: str = _AGENT_OPTS["config"],
    prompt_for_key: bool = typer.Option(
        None,
        "--prompt-for-key/--no-prompt-for-key",
...
def verify_live_agents(
    ...
    if not models:
        console.print(
            f"[red]Error: Live model discovery returned no models. "
    ...
    q1 = (
        "Why is LSTM appropriate for a 600-row tabular multiclass dataset "
        "when no temporal sequence structure was detected? "
        "Compare it with MLP, XGBoost and Wide & Deep."
    )
    ctx1 = AgentContext(
        agent="ArchitectureReviewAgent",
        recommendation="lstm",
        reason="LSTM was chosen by wizard.",
        risk_if_ignored="Potential suboptimal performance.",
        alternatives=[{"family": "mlp"}, {"family": "xgboost"}, {"family": "wide_deep"}],
        dataset_summary="600 rows x 23 cols",
        checkpoint="architecture",
        business_context="Robust generalization and explainability",
        reviewer_clarification="Compare with other architectures",
    ...
    in_tokens1 = getattr(llm, "last_input_tokens", 0)
    out_tokens1 = getattr(llm, "last_output_tokens", 0)

    console.print(f"  [cyan]Answer:[/cyan]\n{ans1}\n")
    console.print("  [cyan]Telemetry:[/cyan]")
    console.print(f"    Response ID : {response_id1}")
    console.print(f"    Latency     : {latency1:.4f}s")
    console.print(f"    Tokens      : {in_tokens1} (in) / {out_tokens1} (out)")

    # Assertions
    if not ans1 or ans1 == "Deterministic answer fallback":
        console.print("[red]Error: ArchitectureReviewAgent answer is empty or fallback.[/red]")
        raise typer.Exit(code=1)
    if not response_id1:
        console.print("[red]Error: Response ID is empty.[/red]")
        raise typer.Exit(code=1)
    if latency1 <= 0:
        console.print("[red]Error: Measured latency is not positive.[/red]")
        raise typer.Exit(code=1)
    if in_tokens1 <= 0 or out_tokens1 <= 0:
        console.print("[red]Error: Token counts are zero.[/red]")
        raise typer.Exit(code=1)

    # 4. Query FeatureEngineeringAgent
    q2 = (
        "Why is median imputation preferable to KNN or MICE when two columns "
        "have approximately 4.3% missingness? State assumptions and trade-offs."
    )
    ctx2 = AgentContext(
        agent="FeatureEngineeringAgent",
        recommendation="median imputation",
        reason="Low missingness fraction.",
        risk_if_ignored="Missingness can cause downstream issues.",
        alternatives=None,
        dataset_summary="600 rows x 23 cols",
        checkpoint="fe",
        business_context="Imputation selection validation",
        reviewer_clarification="State assumptions and trade-offs",
        task_type="multiclass_classification",
        model_name="lstm",
    )

    console.print(f"\n[bold]Querying FeatureEngineeringAgent with '{selected_model}'...[/bold]")
    ex2 = ask_agent("FeatureEngineeringAgent", q2, ctx2, session, llm=llm, llm_connected=True)

    ans2 = ex2.answer.strip()
    response_id2 = getattr(llm, "last_response_id", "")
    latency2 = getattr(llm, "last_latency_seconds", 0.0)
    in_tokens2 = getattr(llm, "last_input_tokens", 0)
    out_tokens2 = getattr(llm, "last_output_tokens", 0)

    console.print(f"  [cyan]Answer:[/cyan]\n{ans2}\n")
    console.print("  [cyan]Telemetry:[/cyan]")
    console.print(f"    Response ID : {response_id2}")
    console.print(f"    Latency     : {latency2:.4f}s")
    console.print(f"    Tokens      : {in_tokens2} (in) / {out_tokens2} (out)")

    # Assertions
    if not ans2 or ans2 == "Deterministic answer fallback":
        console.print("[red]Error: FeatureEngineeringAgent answer is empty or fallback.[/red]")
        raise typer.Exit(code=1)
    if not response_id2:
        console.print("[red]Error: Response ID is empty.[/red]")
        raise typer.Exit(code=1)
    if latency2 <= 0:
        console.print("[red]Error: Measured latency is not positive.[/red]")
        raise typer.Exit(code=1)
    if in_tokens2 <= 0 or out_tokens2 <= 0:
        console.print("[red]Error: Token counts are zero.[/red]")
        raise typer.Exit(code=1)
