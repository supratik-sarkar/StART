...
def _table(title: str, columns: list[str], header_style: str = "bold") -> Any:
    ...
    t = Table(
        title=title, title_style="bold", header_style=header_style, title_justify="left", show_lines=False
    )
    for c in columns:
        t.add_column(c)
    return t
...
def dataset_discovery_table(
    ...
    t.add_row(
        "Leakage candidates", ", ".join(store.leakage_candidates) if store.leakage_candidates else "none"
    )
    t.add_row("High-correlation pairs", str(len(store.correlations)))
    tr, te, oo = split_props
    t.add_row("Proposed split (train/test/OOS)", f"{tr:.0%} / {te:.0%} / {oo:.0%}")
    return t
...
def metrics_table(metrics_by_split: dict[str, dict[str, float]]) -> Any:
    ...
    for split, m in metrics_by_split.items():
        row = [split] + [f"{m[k]:.4f}" if isinstance(m.get(k), (int, float)) else "—" for k in present]
        t.add_row(*row)
    return t
...
def tuning_table(trials: list[dict[str, Any]]) -> Any:
    ...
    for tr in trials:
        ...
        row = (
            [str(tr.get("trial"))]
            + [str(p.get(k, "")) for k in keys]
            + [f"{tr.get('validation_metric', float('nan')):.4f}", str(tr.get("status"))]
        )
        t.add_row(*row)
    return t
...
def importance_table(rows: list[dict[str, Any]], n: int = 20) -> Any:
    ...
    for r in rows[:n]:
        t.add_row(
            str(r.get("rank")), str(r.get("feature")), str(r.get("importance")), str(r.get("direction", ""))
        )
    return t
...
def sensitivity_ranking_table(rows: list[dict[str, Any]]) -> Any:
    ...
    t = _table(
        "ValidationAgent — feature sensitivity ranking", ["Rank", "Feature", "Max |drift|", "Risk level"]
    )
...
def shock_table(rows: list[dict[str, Any]], features: list[str] | None = None) -> Any:
    ...
    for f in feats:
        row = [f] + [(f"{grid.get(f, {}).get(int(s * 100), 0.0):+.4f}") for s in shocks]
        t.add_row(*row)
    return t
...
_ADAPTER_COLORS = {
    "OPA": "bright_red",
    "MCP Server": "bright_cyan",
    "MCP SDK": "cyan",
    "MCP Inspector": "bright_blue",
    "Langfuse": "bright_magenta",
    "OpenTelemetry": "bright_green",
    "Garak": "red",
    "Promptfoo": "yellow",
    "NeMo Guardrails": "bright_yellow",
    "DeepEval": "green",
    "LangSmith": "magenta",
    "Phoenix": "bright_cyan",
}
...
def adapter_inventory_table(control_surface: list[dict[str, Any]]) -> Any:
    ...
    t = _table(
        ...
        ["Adapter", "Status", "Purpose", "Runtime (s)", "Artifacts", "Evidence", "Install / fallback"],
    )
    style = {"complete": "green", "available": "green", "not_installed": "yellow", "error": "red"}
...
def llm_activation_panel(activation: Any) -> Any:
    ...
    return Panel(
        grid,
        title="[bold]LLM activation[/bold]",
...
def challenge_log_table(challenges: list[dict[str, Any]]) -> Any:
    ...
    t = _table("Reviewer challenge log", ["Status", "Agent", "Challenge", "Evidence used"])
...
def _impact_for(key: str, choice: str, effective: str, recommended: str = "") -> str:
    ...
    k = key.lower()
    if k == "architecture":
        is_override = choice in ("override", "overridden", "keep", "modify", "reject") or (
            bool(recommended) and bool(effective) and effective != recommended
        )
        if is_override:
    ...
    if "correlation_pruning" in k:
        return (
            "kept all features (no pruning)"
            if choice == "reject"
            else "dropped highly-correlated features before training"
        )
    if "outlier" in k:
        return "outliers left untouched" if choice == "reject" else "outlier handling applied where wired"
    if "scaling" in k:
...
def kfold_table(kfold: Any) -> Any:
    ...
    t = _table(
        f"K-fold tuning — {getattr(kfold, 'n_folds', '?')}-fold ({method}), primary metric: {metric}",
...
def decision_ledger_markdown(decisions: list[dict[str, Any]]) -> str:
    ...
    for d in decisions:
        ...
        lines.append(
            f"| {d.get('key')} | {d.get('recommended')} | {d.get('effective')} | {status} | {ev} | {impact} |"
        )
    return "\n".join(lines) + "\n"
