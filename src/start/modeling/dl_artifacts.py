...
def render_loss_curve_artifact(
    ...
    train_pts = " ".join(
        f"{x_scale(ep):.1f},{y_scale(loss_val):.1f}" for ep, loss_val in zip(epochs, train_loss, strict=False)
    )
    val_pts = " ".join(
        f"{x_scale(ep):.1f},{y_scale(loss_val):.1f}" for ep, loss_val in zip(epochs, val_loss, strict=False)
    )
    ...
    for i in range(5):
        ...
        y_pos = y_scale(y_val)
        svg_lines.append(
            f'  <line x1="{margin["left"]}" y1="{y_pos:.1f}" x2="{width - margin["right"]}" y2="{y_pos:.1f}" stroke="#21262d" stroke-dasharray="3,3"/>'
        )
        svg_lines.append(
            f'  <text x="{margin["left"] - 10}" y="{y_pos + 4:.1f}" fill="#8b949e" font-size="10" text-anchor="end">{y_val:.3f}</text>'
        )

    for ep in epochs:
        x_pos = x_scale(ep)
        svg_lines.append(
            f'  <line x1="{x_pos:.1f}" y1="{margin["top"]}" x2="{x_pos:.1f}" y2="{height - margin["bottom"]}" stroke="#161b22"/>'
        )
        svg_lines.append(
            f'  <text x="{x_pos:.1f}" y="{height - margin["bottom"] + 18}" fill="#8b949e" font-size="11" text-anchor="middle">Ep {ep}</text>'
        )

    svg_lines.append(f'  <polyline fill="none" stroke="#58a6ff" stroke-width="2.5" points="{train_pts}"/>')
    svg_lines.append(
        f'  <polyline fill="none" stroke="#f0883e" stroke-width="2.5" stroke-dasharray="5,4" points="{val_pts}"/>'
    )

    for ep, loss_val in zip(epochs, train_loss, strict=False):
        svg_lines.append(
            f'  <circle cx="{x_scale(ep):.1f}" cy="{y_scale(loss_val):.1f}" r="4" fill="#58a6ff"/>'
        )
    for ep, loss_val in zip(epochs, val_loss, strict=False):
        svg_lines.append(
            f'  <circle cx="{x_scale(ep):.1f}" cy="{y_scale(loss_val):.1f}" r="4" fill="#f0883e"/>'
        )

    leg_x = width - margin["right"] - 180
    svg_lines.append(
        f'  <rect x="{leg_x}" y="{margin["top"] + 10}" width="170" height="55" fill="#161b22" rx="4" stroke="#30363d"/>'
    )
    svg_lines.append(
        f'  <line x1="{leg_x + 12}" y1="{margin["top"] + 25}" x2="{leg_x + 35}" y2="{margin["top"] + 25}" stroke="#58a6ff" stroke-width="2.5"/>'
    )
    svg_lines.append(
        f'  <text x="{leg_x + 42}" y="{margin["top"] + 29}" fill="#c9d1d9" font-size="11">Train Loss (BCE)</text>'
    )
    svg_lines.append(
        f'  <line x1="{leg_x + 12}" y1="{margin["top"] + 45}" x2="{leg_x + 35}" y2="{margin["top"] + 45}" stroke="#f0883e" stroke-width="2.5" stroke-dasharray="5,4"/>'
    )
    svg_lines.append(
        f'  <text x="{leg_x + 42}" y="{margin["top"] + 49}" fill="#c9d1d9" font-size="11">Val Loss (BCE)</text>'
    )
...
def render_roc_pr_curve_artifact(
    ...
    svg_lines.append(
        f'  <rect x="{p1_x}" y="{p1_y}" width="{p1_w}" height="{p1_h}" fill="#161b22" stroke="#30363d"/>'
    )
    svg_lines.append(
        f'  <text x="{p1_x + p1_w // 2}" y="{p1_y - 10}" fill="#c9d1d9" font-size="12" font-weight="bold" text-anchor="middle">ROC Curve</text>'
    )
    svg_lines.append(
        f'  <line x1="{p1_x}" y1="{p1_y + p1_h}" x2="{p1_x + p1_w}" y2="{p1_y}" stroke="#484f58" stroke-dasharray="4,4"/>'
    )
    ...
    svg_lines.append(
        f'  <polyline fill="none" stroke="#2ea043" stroke-width="2.5" points="{" ".join(roc_pts)}"/>'
    )
    svg_lines.append(
        f'  <text x="{p1_x + p1_w - 10}" y="{p1_y + p1_h - 15}" fill="#2ea043" font-size="11" text-anchor="end">AUC = {auroc:.4f}</text>'
    )

    p2_x, p2_y, p2_w, p2_h = 420, 60, 290, 260
    svg_lines.append(
        f'  <rect x="{p2_x}" y="{p2_y}" width="{p2_w}" height="{p2_h}" fill="#161b22" stroke="#30363d"/>'
    )
    svg_lines.append(
        f'  <text x="{p2_x + p2_w // 2}" y="{p2_y - 10}" fill="#c9d1d9" font-size="12" font-weight="bold" text-anchor="middle">Precision-Recall Curve</text>'
    )
    ...
    svg_lines.append(
        f'  <polyline fill="none" stroke="#388bfd" stroke-width="2.5" points="{" ".join(pr_pts)}"/>'
    )
...
def render_feature_importance_artifact(
    ...
    for i, (feat, imp) in enumerate(pairs):
        ...
        svg_lines.append(
            f'  <text x="{margin["left"] - 12}" y="{y_pos + bar_h * 0.75:.1f}" fill="#c9d1d9" font-size="12" text-anchor="end">{_svg_escape(feat)}</text>'
        )
        svg_lines.append(
            f'  <rect x="{margin["left"]}" y="{y_pos:.1f}" width="{bw:.1f}" height="{bar_h:.1f}" fill="#388bfd" rx="3"/>'
        )
        svg_lines.append(
            f'  <text x="{margin["left"] + bw + 8:.1f}" y="{y_pos + bar_h * 0.75:.1f}" fill="#58a6ff" font-size="11">{imp:.4f}</text>'
        )
