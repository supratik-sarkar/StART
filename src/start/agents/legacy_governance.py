    ...
    def review(self, records: list[Any]) -> tuple[bool, list[str]]:
        ...
        for r in records:
            if r.status == Status.FAIL:
                ok = False
                items.append(f"Test '{r.test_name}' failed and requires documented disposition. [{r.evidence_id}]")
    ...
    def challenge(self, records: list[Any]) -> list[str]:
        ...
        for r in records:
            ...
            if r.test_id == "preprocessing.feature_drift":
                challenges.append(f"Significant feature drift observed on OOS or development cohort. [{r.evidence_id}]")
                challenges.append(f"Check split sampling strategy to mitigate dataset shift. [{r.evidence_id}]")
...
class EvidenceCriticAgent:
    ...
    def _validate_text(self, text: str, records: list[Any]) -> list[Any]:
        ...
        for citation in citations:
            if citation.startswith("EV-") or citation.startswith("FE-") or citation.startswith("ARCH-") or citation.startswith("TUNE-"):
        ...
        if has_fails:
            ...
            if "ready for sign-off" in lower_text or "ready for signoff" in lower_text or "is ready" in lower_text:
...
class ReviewPlannerAgent:
    ...
    def plan(self, model_meta: Any, dataset: Any) -> Any:
        ...
        if self.config and hasattr(self.config, "test_families") and hasattr(self.config.test_families, "enabled"):
...
class TestRouterAgent:
    def route(self, plan: Any) -> tuple[Any, list[str]]:
        ...
        for t in plan.planned_tests:
            if t.test_id in known_ids or t.test_id.startswith("execution.") or t.test_id.startswith("discovery.") or t.test_id.startswith("split.") or t.test_id.startswith("feature_engineering."):
...
class NarrativeAgent:
    ...
    def _template_narrative(self, run_id: str, records: list[Any]) -> Any:
        ...
        findings = [
            f"Review of validation tests shows nominal behavior. [{r.evidence_id}]"
            for r in records
        ]
        
        limitations = [
            f"Evaluation limited to the metrics verified in the run. [{r.evidence_id}]"
            for r in records
        ]
        
        next_steps = [
            f"Continue continuous model monitoring. [{r.evidence_id}]"
            for r in records
        ]
