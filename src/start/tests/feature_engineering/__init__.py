...
__all__ = [
    "plan", "imputation", "scaling", "numeric_transform", "winsorization",
    "categorical_encoding", "rare_category_grouping", "woe_iv", "monotonic_binning",
    "interactions", "temporal_features", "aggregation_features", "pca_transform",
    "selection", "fitting_scope_audit", "IV_BANDS",
]
...
IV_BANDS: tuple[tuple[float, str], ...] = (
    (0.02, "useless"), (0.10, "weak"), (0.30, "medium"), (0.50, "strong"),
...
def _run(test_id: str, name: str, executor: Any, ctx: TestContext, **kwargs: Any) -> TestResult:
    ...
    try:
        ...
        return TestResult(
            test_id=test_id, test_name=name, status=Status.ERROR, params=kwargs,
...
@register_test(
    ...
    requires=("train",),
    default_params={"steps": (), "fitting_scope": FittingScope.TRAIN_ONLY,
                    "temporal_policy": "causal_only"},
...
def plan(
    ...
    return TestResult(
        ...
        interpretation=(
            ...
            if not mutated
            else f"PLAN MUTATED during execution: recorded {str(recorded)[:16]}…, "
                 f"computed {plan_hash[:16]}…"
        ),
        limitations=[
...
# --------------------------------------------------------------------------- #
@register_test(
    "feature_engineering.imputation", family="feature_engineering",
    name="Imputation", requires=("train",),
    default_params={"strategy": "median", "add_indicator": False},
    context_type="tabular", risk_stripes=_STRIPES,
    risk_dimensions=("data_quality_lineage",), object_kinds=_OBJECTS,
)
def imputation(ctx: TestContext, strategy: str = "median",
               add_indicator: bool = False, fill_value: Any = 0) -> TestResult:
    """Fill missing values from training statistics only."""
    return _run("feature_engineering.imputation", "Imputation", run_imputation, ctx,
...
@register_test(
    "feature_engineering.scaling", family="feature_engineering",
    name="Scaling", requires=("train",), default_params={"method": "standard"},
    context_type="tabular", risk_stripes=_STRIPES,
    risk_dimensions=("implementation_verification",), object_kinds=_OBJECTS,
)
def scaling(ctx: TestContext, method: str = "standard") -> TestResult:
    """Scale numeric features using training statistics only."""
    return _run("feature_engineering.scaling", "Scaling", run_scaling, ctx,
                method=method,
                exclude=(ctx.target_column, ctx.score_column, ctx.prediction_column))


@register_test(
    "feature_engineering.numeric_transform", family="feature_engineering",
    name="Numeric transform", requires=("train",),
    default_params={"method": "yeo_johnson"},
    context_type="tabular", risk_stripes=_STRIPES,
    risk_dimensions=("assumption_validity",), object_kinds=_OBJECTS,
)
def numeric_transform(ctx: TestContext, method: str = "yeo_johnson") -> TestResult:
    ...
    return _run("feature_engineering.numeric_transform", "Numeric transform",
                run_numeric_transform, ctx, method=method,
                exclude=(ctx.target_column, ctx.score_column, ctx.prediction_column))


@register_test(
    "feature_engineering.winsorization", family="feature_engineering",
    name="Winsorization", requires=("train",),
    default_params={"method": "iqr", "k": 1.5},
    context_type="tabular", risk_stripes=_STRIPES,
    risk_dimensions=("data_quality_lineage",), object_kinds=_OBJECTS,
)
def winsorization(ctx: TestContext, method: str = "iqr", k: float = 1.5,
                  lower_pct: float = 1.0, upper_pct: float = 99.0,
                  z_threshold: float = 3.0) -> TestResult:
    """Clip extremes to bounds learned on train. A transformation, unlike
    ``preprocessing.outliers`` which only reports."""
    return _run("feature_engineering.winsorization", "Winsorization", run_winsorization,
                ctx, method=method, k=k, lower_pct=lower_pct, upper_pct=upper_pct,
                z_threshold=z_threshold,
                exclude=(ctx.target_column, ctx.score_column, ctx.prediction_column))


@register_test(
    "feature_engineering.categorical_encoding", family="feature_engineering",
    name="Categorical encoding", requires=("train",),
    default_params={"method": "onehot", "n_folds": 5, "smoothing": 10.0},
    context_type="tabular", risk_stripes=_STRIPES,
    risk_dimensions=("implementation_verification", "use_boundary"),
    object_kinds=_OBJECTS,
)
def categorical_encoding(ctx: TestContext, method: str = "onehot", n_folds: int = 5,
                         smoothing: float = 10.0) -> TestResult:
    """Encode categoricals. ``target`` encoding is out-of-fold on the train side."""
    return _run("feature_engineering.categorical_encoding", "Categorical encoding",
                run_categorical_encoding, ctx, method=method,
                target_column=ctx.target_column, n_folds=n_folds, smoothing=smoothing,
                seed=ctx.seed,
                exclude=(ctx.score_column, ctx.prediction_column))


@register_test(
    "feature_engineering.rare_category_grouping", family="feature_engineering",
    name="Rare category grouping", requires=("train",),
    default_params={"min_pct": 1.0},
    context_type="tabular", risk_stripes=_STRIPES,
    risk_dimensions=("data_quality_lineage",), object_kinds=_OBJECTS,
)
def rare_category_grouping(ctx: TestContext, min_pct: float = 1.0,
                           other_label: str = "__OTHER__") -> TestResult:
    """Collapse infrequent levels using training frequencies only."""
    return _run("feature_engineering.rare_category_grouping", "Rare category grouping",
                run_rare_category_grouping, ctx, min_pct=min_pct, other_label=other_label,
                exclude=(ctx.target_column, ctx.score_column, ctx.prediction_column))


@register_test(
    "feature_engineering.woe_iv", family="feature_engineering",
    name="Weight of evidence / IV", requires=("train", "target_column"),
    default_params={"bins": 10, "min_bin_pct": 0.05, "smoothing": 0.5},
    context_type="tabular", risk_stripes=("credit", "model"),
    risk_dimensions=("conceptual_soundness", "use_boundary"),
    object_kinds=("scorecard", "statistical_model"),
)
def woe_iv(ctx: TestContext, bins: int = 10, min_bin_pct: float = 0.05,
           smoothing: float = 0.5, n_folds: int = 5,
           iv_suspicious: float = 0.5) -> TestResult:
    ...
    train, test, oos = _cohorts(ctx)
    try:
        result = run_woe_iv(train, test, oos, target_column=ctx.target_column,
                            bins=bins, min_bin_pct=min_bin_pct, smoothing=smoothing,
                            n_folds=n_folds, seed=ctx.seed,
                            exclude=(ctx.score_column, ctx.prediction_column))
    except ValueError as exc:
    ...
    extra = {"max_iv": round(max_iv, 6), "max_iv_feature": top,
             "max_iv_band": band(max_iv), "n_features_binned": len(ivs)}
    for column, value in sorted(ivs.items()):
        extra[f"iv.{column}"] = round(float(value), 6)

    emitted = _emit(
        "feature_engineering.woe_iv", "Weight of evidence / IV", result, extra,
        ["IV bands (0.02 / 0.10 / 0.30 / 0.50) are credit-scoring industry practice "
         ...
         "An IV above the suspicious threshold is more often leakage than a strong "
         "feature."],
    )
...
@register_test(
    "feature_engineering.monotonic_binning", family="feature_engineering",
    name="Monotonic binning", requires=("train", "target_column"),
    default_params={"max_bins": 10, "min_bin_pct": 0.05, "direction": "auto"},
    context_type="tabular", risk_stripes=("credit", "model"),
    risk_dimensions=("conceptual_soundness",),
    object_kinds=("scorecard", "statistical_model"),
)
def monotonic_binning(ctx: TestContext, max_bins: int = 10, min_bin_pct: float = 0.05,
                      direction: str = "auto") -> TestResult:
    """Bin numeric features so the target rate is monotone. Documented merge rule."""
    train, test, oos = _cohorts(ctx)
    try:
        result = run_monotonic_binning(train, test, oos, target_column=ctx.target_column,
    ...
    extra = {
        ...
        "mean_bins": round(
            float(np.mean([len(v) - 1 for v in edges.values()])) if edges else 0.0, 4
        ),
        "directions": ", ".join(
...
@register_test(
    "feature_engineering.interactions", family="feature_engineering",
    name="Feature interactions", requires=("train",),
    default_params={"method": "product", "max_features": 50,
                    "denominator_floor": 1e-6},
    context_type="tabular", risk_stripes=_STRIPES,
    risk_dimensions=("conceptual_soundness",), object_kinds=_OBJECTS,
)
def interactions(ctx: TestContext, method: str = "product", max_features: int = 50,
                 denominator_floor: float = 1e-6) -> TestResult:
    """Pairwise interactions, deterministically named and hard-capped in count."""
    return _run("feature_engineering.interactions", "Feature interactions",
                run_interactions, ctx, method=method, max_features=max_features,
                denominator_floor=denominator_floor,
                exclude=(ctx.target_column, ctx.score_column, ctx.prediction_column))


@register_test(
    "feature_engineering.temporal_features", family="feature_engineering",
    name="Temporal features", requires=("train", "timestamp_column"),
    default_params={"features": ("hour", "dow", "month", "days_since")},
    context_type="tabular", risk_stripes=_STRIPES,
    risk_dimensions=("use_boundary", "data_quality_lineage"), object_kinds=_OBJECTS,
)
def temporal_features(
    ...
    return _run("feature_engineering.temporal_features", "Temporal features",
                run_temporal_features, ctx, timestamp_column=ctx.timestamp_column,
                features=features,
                exclude=(ctx.target_column, ctx.score_column, ctx.prediction_column))


@register_test(
    "feature_engineering.aggregation_features", family="feature_engineering",
    name="Aggregation features",
    requires=("train", "entity_id_column", "timestamp_column"),
    default_params={"windows": (7, 30, 90)},
    context_type="tabular", risk_stripes=_STRIPES,
    risk_dimensions=("use_boundary",), object_kinds=_OBJECTS,
)
def aggregation_features(ctx: TestContext,
                         windows: tuple[int, ...] = (7, 30, 90)) -> TestResult:
    """Rolling per-entity statistics, strictly backward-looking."""
    return _run("feature_engineering.aggregation_features", "Aggregation features",
                run_aggregation_features, ctx,
                entity_id_column=ctx.entity_id_column,
                timestamp_column=ctx.timestamp_column, windows=windows,
                exclude=(ctx.target_column, ctx.score_column, ctx.prediction_column))


@register_test(
    "feature_engineering.pca_transform", family="feature_engineering",
    name="PCA transform", requires=("train",),
    default_params={"n_components": 0.95, "whiten": False},
    context_type="tabular", risk_stripes=_STRIPES,
    risk_dimensions=("conceptual_soundness", "explainability"), object_kinds=_OBJECTS,
)
def pca_transform(ctx: TestContext, n_components: float | int = 0.95,
                  whiten: bool = False) -> TestResult:
    ...
    train, test, oos = _cohorts(ctx)
    try:
        result = run_pca_transform(train, test, oos, n_components=n_components,
                                   whiten=whiten, seed=ctx.seed,
                                   exclude=(ctx.target_column, ctx.score_column,
                                            ctx.prediction_column))
    except ValueError as exc:
    ...
    return _emit(
        "feature_engineering.pca_transform", "PCA transform", result, extra,
...
@register_test(
    "feature_engineering.selection", family="feature_engineering",
    name="Feature selection", requires=("train",),
    default_params={"method": "mutual_info", "top_k": None, "threshold": None},
    context_type="tabular", risk_stripes=_STRIPES,
    risk_dimensions=("conceptual_soundness", "use_boundary"), object_kinds=_OBJECTS,
)
def selection(ctx: TestContext, method: str = "mutual_info", top_k: int | None = None,
              threshold: float | None = None, vif_threshold: float = 10.0) -> TestResult:
    ...
    train, test, oos = _cohorts(ctx)
    try:
        result = run_selection(train, test, oos, method=method,
                               target_column=ctx.target_column, top_k=top_k,
                               threshold=threshold, vif_threshold=vif_threshold,
                               seed=ctx.seed, model=ctx.model,
                               exclude=(ctx.score_column, ctx.prediction_column))
    except ValueError as exc:
        return _skip("feature_engineering.selection", "Feature selection", str(exc),
                     method=method)
...
@register_test(
    "feature_engineering.fitting_scope_audit", family="feature_engineering",
    name="Fitting-scope audit", requires=("train",),
...
def fitting_scope_audit(ctx: TestContext, steps: tuple[str, ...] = ()) -> TestResult:
    ...
    if test is None:
        return _skip(
            "feature_engineering.fitting_scope_audit", "Fitting-scope audit",
    ...
    for name, executor, kwargs in _AUDITABLE:
        if name not in selected:
            continue
        call = dict(kwargs)
        needs_target = name in {"categorical_encoding", "woe_iv", "monotonic_binning",
                                "selection"}
        if needs_target and not ctx.target_column:
            skipped.append(f"{name} (no target column)")
            continue
        call["exclude"] = exclude if needs_target else (
            ctx.target_column, ctx.score_column, ctx.prediction_column
        )
        audit = audit_executor(
            executor, train, test, oos, step=name,
    ...
    return TestResult(
        ...
        limitations=[
            "Checks the transformations in this registry. A bespoke step outside it is "
            "not covered.",
