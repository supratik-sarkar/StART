...
LEGACY_METADATA: dict[str, tuple[str, tuple[str, ...], tuple[str, ...], tuple[str, ...], str]] = {
    ...
    "supervised.cohort_metrics_comparison": (
        "tabular",
        _MODEL_CREDIT,
        ("overfitting_generalisation", "discriminatory_power"),
        _ML_OBJECTS,
    ...
    "preprocessing.outliers": (
        ...
        ("data_quality_lineage",),
        _DATA_OBJECTS,
        "Emits Tukey-fence outlier rates per column with thresholds and, when enabled, a boxplot artifact.",
    ),
    "preprocessing.split_diagnostics": (
        "tabular",
        _MODEL_CREDIT,
        ("use_boundary", "data_quality_lineage"),
        _DATA_OBJECTS,
    ...
    "genai.citation_coverage": (
        "tabular",
        ("ai_genai",),
        ("documentation_completeness", "output_consumption"),
...
DECLINED: tuple[tuple[str, str, str], ...] = (
    (
        "supervised.classification_metrics",
        "outcomes_analysis",
        "Classification statistics against the modelling target are not realised post-deployment outcomes.",
    ),
    (
        "supervised.top_decile_lift",
        "outcomes_analysis",
    ...
    (
        "supervised.discrimination",
        "accuracy_calibration",
        "AUC is invariant to monotone rescaling, so it cannot evidence probability quality.",
    ),
    (
        "xai.feature_sensitivity",
        "stress_scenario",
    ...
    ("*", "bias_fairness", "No legacy engine computes disparity across protected groups. A genuine gap."),
    (
        "*",
        "benchmarking",
